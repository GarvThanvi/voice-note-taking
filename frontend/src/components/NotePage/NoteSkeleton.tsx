    ...
    return (
      <>
        {skeletonItems(count).map((i) => (
          <article
            key={i}
            className="flex items-center gap-5 p-5 rounded-xl border border-border bg-surface animate-pulse"
          >
  ...
  return (
    <>
      {skeletonItems(count).map((i) => (
        <article
          key={i}
          className="min-h-[235px] rounded-xl border border-border bg-surface p-5 flex flex-col animate-pulse"
        >
